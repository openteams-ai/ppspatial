"""Build ppspatial as an importable CPython extension module.

Produces `ppspatial_native.<ext-suffix>` in the repository root: a real
extension module whose attributes are numpy.ufunc objects registered from
the compiled kernels (postpyc `ext_module=True` output). This target is
intentionally separate from the plain C shared library built by
...
PACKAGE_ENTRY = REPO_ROOT / "ppspatial" / "__init__.py"
